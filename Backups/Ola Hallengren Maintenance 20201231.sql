...
DECLARE @BackupDirectory nvarchar(max)     = N'\\prdsqlbus2\sql_data_backup$'        -- Specify the backup root directory. If no directory is specified, the default backup directory is used.
DECLARE @CleanupTime int                   = 144        -- Time in hours, after which backup files are deleted. If no time is specified, then no backup files are deleted.
