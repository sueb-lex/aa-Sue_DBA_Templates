...
/****** Object:  StoredProcedure [dbo].[usp_EmailSQLServerHealth]    Script Date: 06/17/2019 02:39:33 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_EmailSQLServerHealth]
(
@ServerIP VARCHAR(100) = '172.30.11.196', -- SQL Server 2005 Database Server IP Address
@Project VARCHAR(100) = 'SQL Server Health Check', -- Name of project or client 
@Recepients VARCHAR(2000) = 'DBA_Notification@lexingtonky.gov', -- Recepient(s) of this email (; separated in case of multiple recepients).
--@MailProfile VARCHAR(100) = 'Public DBMail Profile', -- Mail profile name which exists on the target database server
@Owner VARCHAR(200) = 'sueb@lexingtonky.gov' -- Owner, basically name/email of the DBA responsible for the server
)  

/*

exec dbo.usp_EmailSQLServerHealth '10.10.10.10',  'MYProject', 'myself@mycompany.com', 'TestMailProfile', 'My Self'

http://www.sqlservercentral.com/articles/Automating+SQL+Server+Health+Checks/68910/
Sue Boorman (01/28/2010)
...
INSERT #jobs_status EXEC msdb.dbo.sp_help_job --replaced with code below
/*
