USE [master]
GO
/****** Object:  StoredProcedure [dbo].[expressmaint]    Script Date: 02/03/2014 15:28:38 ******/
SET ANSI_NULLS ON
GO
...
(
   @database      sysname,                   -- database_name | ALL_USER | ALL_SYSTEM | ALL_DB
   ...
   @dbretainval   int = 0,                   -- specifies how many retainunits to keep backup
   ...
   @rptretainval  int = 0,                   -- specifies how many retainunits to keep reports
...
AS
/* http://blog.rdx.com/blog/dba_tips/2014/02/maintaining-your-databases-on-sql-server-express-edition
	...
	03/02/2014		Brandi Dollar		Updates and fixes

	USAGE
	1	Run script to create stored procedure in master database
	2	Create file(s) with .sql extension. Two files need to be created if backing up all user and system databases. The contents will be similar to the below example. Variable explanations are above.
		exec expressmaint
		   @database      = 'MyDatabase', 
		   @optype        = 'DB',
		   @backupfldr    = 'C:\Program Files (x86)\Microsoft SQL Server\MSSQL.1\MSSQL\Backup',
		   @reportfldr    = 'C:\Program Files (x86)\Microsoft SQL Server\MSSQL.1\MSSQL\Backup',
		   @verify        = 1,
		   @dbretainunit  = 'weeks',
		   @dbretainval   = 1,
		   @rptretainunit = 'weeks',
		   @rptretainval  = 1,
		   @report        = 1
	3	Create scheduled task(s). Two tasks need to be created if backing up all user and system databases. Below are examples for different Windows versions. For named instances, prefix "instance_or_server_name" with ".\" (no quotes around either).
		Windows 2003
			Task - Run: "SQLCMD.EXE location with exe name goes here" -S instance_or_server_name -i".sql location goes here"
			Task - Start in: "SQLCMD.EXE folder location only goes here"
		Windows 2008 and 2008 R2
			Action - Program/script: "SQLCMD.EXE location with exe name goes here"
			Action - Add arguments: -S instance_or_server_name -i".sql location goes here"
*/
...
************************/
   ...
   SELECT @versionmajor = CAST(LEFT(CAST(SERVERPROPERTY('ProductVersion') as varchar(128)),CHARINDEX('.',CAST(SERVERPROPERTY('ProductVersion') as varchar(128)))-1) as int),
          @versionminor = CAST(LEFT(SUBSTRING(CAST(SERVERPROPERTY('ProductVersion') as varchar(128)), CHARINDEX('.',CAST(SERVERPROPERTY('ProductVersion') as varchar(128)))+1, 25),CHARINDEX('.',SUBSTRING(CAST(SERVERPROPERTY('ProductVersion') as varchar(128)), CHARINDEX('.',CAST(SERVERPROPERTY('ProductVersion') as varchar(128)))+1, 25))-1) as int),
...
************************/
   ...
   IF @database NOT IN ('ALL_USER','ALL_SYSTEM','ALL_DB')
   BEGIN
   ...
   -- check recovery mode is correct if trying log backup of single database
   IF @database NOT IN ('ALL_USER','ALL_SYSTEM','ALL_DB')
   BEGIN
   ...
   -- raise error if attempting to perform a differential backup of master database
   IF @database = 'master' and @optype = 'DIFF'
   BEGIN
      	RAISERROR('%s is not a valid option for database %s because differential backups cannot be performed',16,1,@optype,@database)
         SET @ret = 1
      	GOTO CLEANUP
   END
   ...
   IF @optype NOT IN ('REINDEX','CHECKDB','REORG')
   BEGIN
      IF UPPER(@dbretainunit) NOT IN ('MINUTES','HOURS','DAYS','WEEKS','MONTHS','COPIES') or @dbretainunit IS NULL
    	BEGIN                   				
   ...
   IF @dbretainval<1 and @optype NOT IN ('REINDEX','CHECKDB','REORG')
 	BEGIN                   				
   ...
   IF (UPPER(@rptretainunit) NOT IN ('MINUTES','HOURS','DAYS','WEEKS','MONTHS','COPIES') or @rptretainunit IS NULL) and @report = 1
 	BEGIN                   				
   ...
   IF @rptretainval<1 and @report = 1
 	BEGIN                   				
...
************************************/
   ...
   IF @database IN ('ALL_USER','ALL_SYSTEM','ALL_DB')
   BEGIN
      ...
      ELSE
         ...
         IF @optype = 'LOG'
            INSERT #databases(dbname)
            SELECT [name] from sys.databases 
            where (database_id in (1,3,4) OR database_id > 4)
            AND recovery_model <> '3'
            AND state = 0
         ELSE IF @optype = 'DIFF'
            INSERT #databases(dbname)
            SELECT [name] from sys.databases 
            where (database_id in (3,4) OR database_id > 4)
            AND state = 0
         ELSE
            INSERT #databases(dbname)
            SELECT [name] from sys.databases
...
************************************/
   ...
   BEGIN
      ...
      -- check @engineedition is express
      -- if @engineedition is not express, the script will complete but functionality may be lost so a warning is raised
      IF @engineedition <> 4
 	BEGIN                   				
   	   RAISERROR('Value for @engineedition indicates this is not SQL Server Express, Express with Tools, or Express with Advanced Services Edition. Functionality may be lost.',10,1,@engineedition)
           SET @output = SPACE(4) + '*** Value for @engineedition indicates this is not SQL Server Express, Express with Tools, or Express with Advanced Services Edition. Functionality may be lost. ***'
           IF @debug = 1 PRINT @output
           IF @report = 1
           BEGIN
              SET @output = SPACE(4) + 'Value for @engineedition indicates this is not SQL Server Express, Express with Tools, or Express with Advanced Services Edition. Functionality may be lost.'
   ...
   BEGIN
      ...
      ************************/
      ...
      BEGIN
         ...
         IF @debug = 1 
		 BEGIN
			PRINT 'DIFFERENTIAL BACKUP : ' + @execmd
		 END
      ...
      ************************/
      ...
      BEGIN
         ...
         BEGIN
            IF (select recovery_model from sys.databases where name = @database) = 3
         	BEGIN
                   SELECT @errormsg = 'Log backups cannot be taken for database ' + @database + ' because it is in SIMPLE recovery mode'
                   SET @output = SPACE(4) + '*** ' + @errormsg + ' ***'
                   IF @debug = 1 PRINT @output
                   IF @report = 1
                   BEGIN
                      SET @output = SPACE(4) + 'Log backups cannot be taken for database ' + @database + ' because it is in SIMPLE recovery mode'
                      EXEC sp_OAMethod @file,'WriteLine',NULL,@output
                   END
            	   FETCH NEXT FROM dcur into @database
         	END
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN CATCH
               ...
               ELSE
                  SET @errormsg = 'Reorganization of indexes on [' + @database + N'].' + @table + ' failed with Native Error : ' + CAST(@err as varchar(10))
