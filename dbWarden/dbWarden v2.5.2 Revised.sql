/*******************************************************************************************************************************************************
**	-----------  Version: 2.5.2 Revised (12/02/2020 sb) ----------- 
**
...
**http://www.sqlservercentral.com/articles/Monitoring/98106/ (SQL Server article that explains how to do it)
**http://sourceforge.net/projects/dbwarden/ (source code)
**http://www.sqlservercentral.com/Forums/Topic1441532-3362-1.aspx (comments about the article)
**http://www.sqlservercentral.com/articles/Automating+SQL+Server+Health+Checks/68910/ (based on this article)
...
EXEC msdb.dbo.sp_add_operator @name=N'SQL_DBA', 
		@enabled=1,
		@email_address=N'DBA_Notification@lexingtonky.gov'--changed from N'CHANGEME'  updated 12/02/2020 sb
END
GO

IF NOT EXISTS (SELECT * FROM msdb.dbo.sysoperators WHERE name = 'SQL_DBA_text')
BEGIN
EXEC msdb.dbo.sp_add_operator @name=N'SQL_DBA_text', 
		@enabled=1,
		@email_address=N'8593616660@1waysms.firstnet.com'--changed from N'CHANGEME'  updated 12/02/2020 sb
END
GO
/*=======================================================================================================================
=============================================DBA DB CREATE===============================================================
...
BEGIN
	...
	ALTER AUTHORIZATION ON DATABASE::[dbWarden] to sa;  -- changed this to set dbowner to 'sa' 12/02/2020 sb
END
GO
/*========================================================================================================================
====================================================DBA TABLES============================================================
...
BEGIN
--changed settings for LogFileAlerts to 1 and LongQueryAlerts to 1 (sb 02/26/2014)	
UPDATE [dbWarden].dbo.DatabaseSettings
SET SchemaTracking = 0,
	LogFileAlerts = 1,
	LongQueryAlerts = 1,
	Reindex = 0
--YOU MUST ADD DATABASES TO THIS LIST WHERE TO WANT TO TRACK SCHEMA CHANGES, OTHERWISE THE SCHEMA CHANGE OBJECTS WILL NOT BE INSTALLED!
--WHERE [DBName] IN ('CHANGEME')

/*	original statement I changed (sb) -see above
UPDATE [dbWarden].dbo.DatabaseSettings
...
BEGIN
	...
	SELECT 'LongRunningJobs',(SELECT email_address FROM msdb..sysoperators WHERE name = 'SQL_DBA'),(SELECT email_address FROM msdb..sysoperators WHERE name = 'SQL_DBA_text') UNION ALL
	SELECT 'LongRunningQueries',(SELECT email_address FROM msdb..sysoperators WHERE name = 'SQL_DBA'),(SELECT email_address FROM msdb..sysoperators WHERE name = 'SQL_DBA_text') UNION ALL
	SELECT 'BlockingAlert',(SELECT email_address FROM msdb..sysoperators WHERE name = 'SQL_DBA'),(SELECT email_address FROM msdb..sysoperators WHERE name = 'SQL_DBA_text') UNION ALL
	SELECT 'LogFiles',(SELECT email_address FROM msdb..sysoperators WHERE name = 'SQL_DBA'),(SELECT email_address FROM msdb..sysoperators WHERE name = 'SQL_DBA_text') UNION ALL
	SELECT 'TempDB',(SELECT email_address FROM msdb..sysoperators WHERE name = 'SQL_DBA'),(SELECT email_address FROM msdb..sysoperators WHERE name = 'SQL_DBA_text') UNION ALL
	SELECT 'HealthReport',(SELECT email_address FROM msdb..sysoperators WHERE name = 'SQL_DBA'),NULL UNION ALL
	SELECT 'CPUAlert',(SELECT email_address FROM msdb..sysoperators WHERE name = 'SQL_DBA'),(SELECT email_address FROM msdb..sysoperators WHERE name = 'SQL_DBA_text')
END
GO
...
**
**	RUN WITHOUT SENDING EMAIL AND WITHOUT RETAINING DATA  --added 12/03/2020 sb
**		EXEC dbo.rpt_HealthReport @Recepients = 'sueb@lexingtonky.gov', @InsertFlag = 0, @EmailFlag = 0
**
...
BEGIN
	...
	CREATE TABLE #REPLINFO (
        ...
        [deletebatchsize_xact] INT, /* added 01/07/2019 sb https://www.sqlservercentral.com/Forums/1441532/dbWarden--A-Free-SQL-Server-Monitoring-Package?PageIndex=4 */
        [deletebatchsize_cmd] INT, /* added 01/07/2019 sb https://www.sqlservercentral.com/Forums/1441532/dbWarden--A-Free-SQL-Server-Monitoring-Package?PageIndex=4 */		
	...
	CREATE TABLE #PUBINFO (
        ...
        publisher NVARCHAR(128) /* added 01/07/2019 sb https://www.sqlservercentral.com/Forums/1441532/dbWarden--A-Free-SQL-Server-Monitoring-Package?PageIndex=4#bm1847505 */
		)
	...
	BEGIN
		...
		SELECT
			...
			CASE
				...
				ELSE '<td width="125" class="c2">' + 'N/A'														
			END +
			CASE
				...
				ELSE '<td width="125" class="c1">' + 'N/A'														
			END +
			CASE
...
EXEC @ReturnCode =  msdb..sp_add_job @job_name=N'dbWarden_HealthReport', 
		@enabled=0, 
		@notify_level_eventlog=2, --changed from 0 to 2 (write to Windows event log on job failure) 12/02/2020 sb
		@notify_level_email=2,  
		@notify_level_netsend=0, 
		@notify_level_page=0, 
		@delete_level=0, 
